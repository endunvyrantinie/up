...
export async function POST(req: Request) {
  try {
    ...
    const categoryCode = process.env.TOYYIBPAY_CATEGORY_CODE; // Now uses the Vercel variable

    const body = new URLSearchParams();
    body.append('userSecretKey', secretKey!);
    body.append('categoryCode', categoryCode!);
    body.append('billName', "D' Mannee Wallet Recharge"); // Updated Name
    ...
    if (data[0] && data[0].BillCode) {
      ...
      throw new Error('Failed to generate D\' Mannee payment URL');
    }
