...
export async function GET(request: NextRequest) {
  try {
    ...
    const level1Refs: ReferralTreeItem[] = referrals
      ...
      .map((r: any) => {
        const referredUser = users.find((u: any) => u.id === r.referredId);
        return {
          id: r.referredId,
    ...
    level1Refs.forEach((l1: any) => {
      const l2 = referrals
        .filter((r: any) => r.referrerId === l1.id && r.level === 1)
        .map((r: any) => {
          const referredUser = users.find((u: any) => u.id === r.referredId);
          return {
            id: r.referredId,
            ...
            commission: referrals
              .filter((ref: any) => ref.referredId === r.referredId && ref.referrerId === user.id && ref.level === 2)[0]?.commission || 0,
            createdAt: r.createdAt,
          } as ReferralTreeItem;
        });
      level2Refs.push(...l2);
    ...
    level2Refs.forEach((l2: any) => {
      const l3 = referrals
        .filter((r: any) => r.referrerId === l2.id && r.level === 1)
        .map((r: any) => {
          const referredUser = users.find((u: any) => u.id === r.referredId);
          return {
            id: r.referredId,
            ...
            level: 3,
            commission: referrals
              .filter((ref: any) => ref.referredId === r.referredId && ref.referrerId === user.id && ref.level === 3)[0]?.commission || 0,
            createdAt: r.createdAt,
          } as ReferralTreeItem;
        });
      level3Refs.push(...l3);
